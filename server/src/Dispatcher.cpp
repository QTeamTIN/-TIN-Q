...
void Dispatcher::processLogin(Packet packet)
{
    ...
    std::string hash = loginPacket.hash();
    int32_t sessionID = serverAPI.callLogin(userName, hash);
...
void Dispatcher::processQuery(Packet packet)
{
    ...
    auto strResp = std::get<1>(res);
    for (int i = 0; i < strResp.size(); ++i)
        response->set_str_args(i, strResp.back());

    response->set_object_id(operation_id);
    output_queue_.push(queryResponse);
}

void Dispatcher::processQueryResponse(Packet packet)
{
    // done in processQuery
}

void Dispatcher::processAlive(Packet packet)
{
    std::cout<<"Alive packet processing\n";
    auto alivePacket = packet.alive();
    int32_t session_id = alivePacket.session_id();
    serverAPI.refreshSession(session_id);
}
